...
{
    public partial class Details2SchemePlus : Form
    {
        CRUD crud = new CRUD();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                DataTable dt_temp = crud.ExecQuery("select CLAIM_SEQ_NO from user_claim_scheme where CLAIM_NUMBER='" + mainFrm.txtClaimNo.Text.ToUpper() + "' and SCHEME_DETAILS='btnDetail2Plus'");
        ...
        public bool call_sp_scheme(string a)
        {
            ...
            try
            {
                ...
                "cl_gender", "cl_age", "cl_plan", "cl_loss_date", "cl_claim_number", "cl_create_date", "cl_remark", "cl_btn_details","cl_seq_no_temp","cl_daignosis" },
                               ...
                               txtRemark2SchemePlus.Text,"btnDetail2Plus","",txtDaignosis.Text  });
        ...
        private void Details2SchemePlus_Load(object sender, EventArgs e)
        {
            DataTable dt_load = crud.ExecQuery("select distinct t.*,CLAIM_NUMBER,SCHEME_DETAILS,CL_DAIGNOSIS,REMARK,INSURED_NAME,POLICY_NUMBER,regexp_replace(TO_CHAR(INCEPTION_DATE,'DD MONTH YYYY'),'\\s+', ' ') INCEPTION_DATE,regexp_replace(TO_CHAR(EXPIRY_DATE,'DD MONTH YYYY'),'\\s+', ' ') EXPIRY_DATE,PREMIUM_STATUS,MEMBER,MEMBER_STATUS,GENDER,to_CHAR(AGE) AGE,PLAN,regexp_replace(TO_CHAR(LOSS_DATE,'DD MONTH YYYY'),'\\s+', ' ') LOSS_DATE from ( " +
                    ...
                    "and SCHEME_DETAILS = 'btnDetail2Plus' and CLAIM_NUMBER = '" + mainFrm.txtClaimNo.Text.ToUpper() + "'");
            
            if (dt_load.Rows.Count > 0)
            {
                txtDaignosis.Text = dt_load.Rows[0]["CL_DAIGNOSIS"].ToString();
                txtRemark2SchemePlus.Text = dt_load.Rows[0]["REMARK"].ToString();
                dtv2 = dt_load.Copy();
        ...
        private void cus_button5_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Details2SchemePlus_Load(sender, e);
                Reports.DetailsSchemePlus2RP rpt = new Reports.DetailsSchemePlus2RP();
        ...
        //private void txtRemark2SchemePlus_KeyPress(object sender, KeyPressEventArgs e)
        //{
        //    enterkeytotab(e);
        //}
