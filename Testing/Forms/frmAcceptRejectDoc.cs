                    ...
                    else if (rdReject.Checked)
                    {
                        if (rdReason1.Checked || rdReason2.Checked || rdReason3.Checked || rdOther.Checked)
                        {
                            ...
                            if (dr == System.Windows.Forms.DialogResult.Yes)
                            {
                                ...
                                cmd.CommandText = "INSERT INTO dbo.tbNoti(NOTI_DETAIL, NOTI_TO, NOTI_DATE, REMARK, NOTI_TYPE) SELECT 'Instruction Note No \"' + DOC_CODE + '\" has been rejected due to \"'+ @reason +'\"', (SELECT USER_NAME FROM dbo.tbDOC_USER WHERE FULL_NAME = CREATE_BY), getdate(), DOC_CODE, '" + CommonFunctions.NotiType.REJECTED + "' FROM dbo.VIEW_DOC_DETAIL WHERE DOC_CODE in (SELECT * FROM FNC_SPLIT('" + SelectedDocCode + "',','))";
                                cmd.Parameters.Add(new SqlParameter("reason", reason));
                                crud.Executing(cmd);
