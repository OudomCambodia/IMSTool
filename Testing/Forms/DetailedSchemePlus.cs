...
{
    public partial class DetailedSchemePlus : Form
    {
        CRUD crud = new CRUD();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                DataTable dt_temp = crud.ExecQuery("select CLAIM_SEQ_NO from user_claim_scheme where CLAIM_NUMBER='" + mainFrm.txtClaimNo.Text.ToUpper() + "' and SCHEME_DETAILS='btnDetailsPlus'");
        ...
        void Calculation()
        {
            ...
            double PYSum = 0;
            double NPSum = 0;
            double TADue = 0, TPDue = 0, TPYDue = 0,TANPDue = 0, pav = 0, ppv = 0, pnp = 0;
            for (int i = 1; i <= 27; i++)
            {
            ...
            pav = IsNum(txtAC27) ? Convert.ToDouble(txtAC27.Text) : 0.00;
            ppv = IsNum(txtPY27) ? Convert.ToDouble(txtPY27.Text) : 0.00;
            pnp = IsNum(txtNP27) ? Convert.ToDouble(txtNP27.Text) : 0.00;
        ...
        public bool call_sp_scheme(string a)
        {
            ...
            try
            {
                ...
                "cl_gender", "cl_age", "cl_plan", "cl_loss_date", "cl_claim_number", "cl_create_date", "cl_remark", "cl_btn_details","cl_seq_no_temp","cl_daignosis" },
                               ...
                               txtRemarkSchemePlus.Text,"btnDetailsPlus","",txtDaignosis.Text  });
        ...
        #region textboxcall
        private void txtAC1_Leave_1(object sender, EventArgs e)
        {
            Calculation();
        }
        ...
        private void DetailedSchemePlus_Load(object sender, EventArgs e)
        {
            DataTable dt_load = crud.ExecQuery("select distinct t.*,CLAIM_NUMBER,SCHEME_DETAILS,CL_DAIGNOSIS,REMARK,INSURED_NAME,POLICY_NUMBER,regexp_replace(TO_CHAR(INCEPTION_DATE,'DD MONTH YYYY'),'\\s+', ' ') INCEPTION_DATE,regexp_replace(TO_CHAR(EXPIRY_DATE,'DD MONTH YYYY'),'\\s+', ' ') EXPIRY_DATE,PREMIUM_STATUS,MEMBER,MEMBER_STATUS,GENDER,to_CHAR(AGE) AGE,PLAN,regexp_replace(TO_CHAR(LOSS_DATE,'DD MONTH YYYY'),'\\s+', ' ') LOSS_DATE from ( " +
                    ...
                    "and SCHEME_DETAILS = 'btnDetailsPlus' and CLAIM_NUMBER = '" + mainFrm.txtClaimNo.Text.ToUpper() + "'");

            if (dt_load.Rows.Count > 0)
            {

                txtDaignosis.Text = dt_load.Rows[0]["CL_DAIGNOSIS"].ToString();
                txtRemarkSchemePlus.Text = dt_load.Rows[0]["REMARK"].ToString();
                dtv2 = dt_load.Copy();
                ...
                for (int i = 1; i <= 4; i++)
                {
                    ...
                    for (int j = 0; j < dt_load.Rows.Count; j++)
                    {
                        ...
                        {
                            //dtPicker.Text = Convert.ToDateTime(dt_load.Rows[j]["FROM_DATE"].ToString());
                            ////dtPicker.Value = DateTime.ParseExact(dt_load.Rows[j]["FROM_DATE"].ToString(), "dd/MM/yyyy", CultureInfo.InvariantCulture);
                            //dtTo.Text = Convert.ToDateTime(dt_load.Rows[j]["TO_DATE_SCHEME"].ToString());
        ...
        private void cus_button5_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DetailedSchemePlus_Load(sender, e);
                Reports.DetailsSchemePlusRP rpt = new Reports.DetailsSchemePlusRP();
        ...
        //private void txtRemarkSchemePlus_KeyPress(object sender, KeyPressEventArgs e)
        //{
        //    enterkeytotab(e);
        //}
