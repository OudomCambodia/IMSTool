...
namespace Testing.Forms
{
...

    {
        ...
        private void bnView_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (tbRequisitionNo.Text == "")
                {
                    ...
                    string[] Keys = new string[] { "sp_type", "sp_requisition_no", "sp_date_from", "sp_date_to" };
                ...
                {
                    string sp_type = "Cl_Requisition_payment_byreq_no";
                    string[] Keys = new string[] { "sp_type", "sp_requisition_no", "sp_date_from", "sp_date_to" };
                    //string[] Values = new string[] { sp_type, dtpFrom.Value.ToString("yyyy/MM/dd"), dtpTo.Value.ToString("yyyy/MM/dd") };
            ...
            try
            {
                ...
                else
                {
                    if (option == 1)
                    {
                        ...
                        FirstSheetClaimReport(dtcopy, wb, ws, ColumnsCount, RowsCount);
                        //Second Sheet
        ...
        private void wbHeading(string[] Heading, IXLWorkbook wb, IXLWorksheet ws, int ColumnsCount, int RowsCount)
        {
            ...
            for (int i = 1; i <= Heading.Length; i++)
            {
                ...
                if (i < Heading.Length)
                {
                    ...
                    ws.Cell(i, 1).Style.Font.FontName = "Century Gothic";
                ...
                else
                {
                    ...
                    ws.Cell(i - 1, i + 3).Style.Font.FontName = "Century Gothic";
            ...
            ws.Cell(3, 8).Style.Font.FontName = "Century Gothic";
        ...
        private void FirstSheetClaimReport(DataTable dtcopy, IXLWorkbook wb, IXLWorksheet ws, int ColumnsCount, int RowsCount,string ExcelFilePath = null)
        {
            ...
            for (int r = 0; r < RowsCount; r++)
            {
                ...
                if (countr == 0)
                    countr = Convert.ToDouble(ws.Cell(r + 7, 8).Value.ToString());

                if (ws.Cell(r + 7, 5).Value.ToString() == ws.Cell(r + 8, 5).Value.ToString())
                    countr += Convert.ToDouble(ws.Cell(r + 8, 8).Value.ToString());

                //need to check to insert one row group by beneficiary name
                if (ws.Cell(r + 7, 5).Value.ToString() != ws.Cell(r + 8, 5).Value.ToString())
                {
                    ws.Row(r + 7).InsertRowsBelow(2);
                    ws.Cell(r + 8, 8).SetValue(string.Format("{0:0.00}", countr));
                    //ws.Cell(r + 8, 8).Style.Fill.SetBackgroundColor(XLColor.CoolGrey);
                    ws.Cell(r + 8, 8).Style.Font.Bold = true;
                    ws.Cell(r + 8, 8).Style.Font.FontSize = 12f;
                    countr = 0;
                    r += 2;
